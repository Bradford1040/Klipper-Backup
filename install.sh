...
check_updates() {
    ...
    git fetch origin devel-v2.5 > /dev/null 2>&1
    local local_hash
    local_hash=$(git rev-parse HEAD)
    local remote_hash
    remote_hash=$(git rev-parse origin/devel-v2.5) # Check against the specific branch
    if [ "$local_hash" = "$remote_hash" ]; then
        ...
        if [[ "$FIX_MODE" == "true" ]] || ask_yn "Proceed with update?"; then
            ...
            if git pull origin devel-v2.5 --ff-only > /dev/null 2>&1; then # Try fast-forward first
                kill $loading_pid &>/dev/null || true
                tput_cnorm # Restore cursor visibility
                wait $loading_pid &>/dev/null || true
                ...
                git reset --hard origin/devel-v2.5 > /dev/null 2>&1 # Reset to remote branch state
